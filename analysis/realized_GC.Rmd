...
Write out grandfathers.
```{r write.out.grandfathers}
save(subdatfield, file = "GC_subdatfield.rda")
```
...
which.ind <- seq_along(subdatfield$grandfather) %in% idx
...
doit.off <- map.factory(routf.granddad, grepl("collected", vars),
    grepl("grandfather", names(routf.granddad$b)), which.ind)
mu.off <- doit.off()
```
...
Now we estimate total change in mean fitness from one generation to the next.
This is a modification of equation (1) in @mean-fitness that accounts for
not all sires being grandsires (`idx`)
```{r total.change}
delta.total <- sum(mu.off * mu.prev[idx]) / sum(mu.prev[idx]) -  mean(mu.prev)
delta.total
```

Now we estimate the part of this total change that is due to environment
(including $G \times E$), equation (2a) of @mean-fitness.
```{r environmental.change}
delta.environ <- mean(mu.par - mu.prev)
delta.environ
```

Now we estimate the part of this total change that is predicted by FFTNS,
equation (3) of @mean-fitness (the current draft).
```{r fftns.change}
delta.fftns <- sum((mu.prev / sum(mu.prev) - 1 / length(mu.prev)) * mu.prev)
delta.fftns
```
Actually we have two other subexperiments to do the same thing with.
So try them too.
```{r fftns.change-too}
delta.fftns.par <- sum((mu.par / sum(mu.par) - 1 / length(mu.par)) * mu.par)
delta.fftns.par
delta.fftns.off <- sum((mu.off / sum(mu.off) - 1 / length(mu.off)) * mu.off)
delta.fftns.off
```
We could use any linear combination of these three FFTNS estimates as our
"official" FFTNS estimate.  (Of course, the first two of these FFTNS estimates
are for the same population in different environments, and the latter two are
for different populations in the same environment.  So none of these are
estimating exactly the same thing.)
For now, we take the first of these.

Now we estimate the remainder of the total change, which is the part of
the genetic change not described by FFTNS.
```{r non.fftns.change}
...
foo
identical(foo.save, foo[ , 1, drop = FALSE])
```
...
# Mean Values (Rather Than Decomposition of DIfferences)

We also want mean values.  This is complicated for offspring in that
we need to correct for population change due to selection in the generation
from the parents.  So we repeat the previous analysis with a different
`doit.combo` function.
```{r combo-too}
doit.combo <- function(moo) {
    stopifnot(is.numeric(moo))
    ...
    idx <- match(sort(unique(subdatfield$grandfather)),
        levels(subdat$paternalID))
    mu.bar.prev <- mean(mu.prev)
    mu.bar.par <- mean(mu.par)
    mu.bar.off <- sum(mu.off * mu.prev[idx]) / sum(mu.prev[idx])
    result <- c(mu.bar.prev, mu.bar.par, mu.bar.off)
    names(result) <- c(paste("parents", myyear), paste("parents", myyearf),
        paste("offspring", myyearf))
    return(result)
}
```
And try this the same way.
```{r test.combo-too}
deltas <- doit.combo(mu.combo)
deltas
```
```{r jack.combo-too,cache=TRUE,dependson=c("vcov-prev","vcov-par","vcov-off")}
<<jack.combo>>
```
```{r delta-table-with-se-too, echo=FALSE, tab.cap="Mean Fitness"}
# note cannot have any dots in chunk label because it screws up caption
# that is, delta.table is fubar but delta-table is OK
<<table>>
knitr::kable(foo, digits=4)
```
Table \@ref(tab:delta-table-with-se-too) shows these in a nice table.
