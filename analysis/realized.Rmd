...
# tricky, only works because each row of m.dads in indicator vector of family
...
my.fun <- function(rout) {
    ...
    dads <- drop(m.dads %*% as.integer(fams))
    which.ind <- match(sort(unique(dads)), dads)
    doit <- map.factory(rout, grepl("collected", vars),
        grepl("paternal", names(rout$b)), which.ind)
    doit() |> structure(names = dads[which.ind])
}
mu.parents <- lapply(rout.parents, function(x) lapply(x, my.fun))
mu.offspring <- lapply(rout.offspring, function(x) lapply(x, my.fun))
```
...
text(1.5 - sin(radians[2]) * delta.text / foo[1],
    ...
    "environment", col = "darkgray", srt = angles[2])
