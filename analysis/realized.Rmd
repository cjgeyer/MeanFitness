...
we did above to make a separate list.  (We could use R function `mapply`
to work on multiple parallel lists, but don't choose to.)  So we do it
again here.
...
One of the main quantities of scientific interest, and the other quantites
...
of $\theta$ and a simple average or $\theta$ with $\mu$ as the unnormalized
weight vector.
...
doit.too(fout)
```
The code chunk above makes Table \@ref(tab:decomp) below.
...
Section \@ref(change-in-mean-fitness-due-to-selection) above
*mutatis mutandis*.
