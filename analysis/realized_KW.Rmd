...
Write out grandfathers for comparison with new document.
```{r write.out.grand}
save(subdatfield, file = "KW_subdatfield.rda")
```
...
vout <- vcov(rout)
range(eigen(vout, symmetric = TRUE, only.values = TRUE)$values)
```
Standard errors do not exist.  Approximate variance-covariance matrix
not positive definite (using the computer's default inexact arithmetic).

Try a smaller model (no maternal effects in model).
```{r reaster.prev, cache=TRUE, cache.extra=key}
rout <- reaster(resp ~ fit + varb,
    list(sire = ~ 0 + fit:paternalID, block = ~ 0 + fit:block),
    pred, fam, varb, id, root, data = redata)
```
And show the results.
```{r reaster.prev.show, dependson="reaster.prev"}
summary(rout)
```
Look at the first step towards standard errors.
```{r reaster.prev.vcov, cache=TRUE, dependson="reaster.prev"}
vout <- vcov(rout)
range(eigen(vout, symmetric = TRUE, only.values = TRUE)$values)
```
...
Dump for debugging.
```{r dump}
save(redata, pred, fam, file = "redata.rda")
```
